...
// Load all AH JSON files in the directory
const allFiles = fs.readdirSync(__dirname);
const files = allFiles
  .filter(file => file.startsWith('adaptive-heuristics-') && file.endsWith('.json'))
  .sort(); // Sort to ensure chronological order

console.log(`Found ${files.length} adaptive heuristics files: ${files.join(', ')}\n`);
